...
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  casadi_fill(w10, 3, 10.);
  /* #70: @18 = @14[3:6] */
  for (rr=w18, ss=w14+3; ss!=w14+6; ss+=1) *rr++ = *ss;
  /* #71: @26 = @22[3:6] */
  for (rr=w26, ss=w22+3; ss!=w22+6; ss+=1) *rr++ = *ss;
  /* #72: @18 = (@18-@26) */
  for (i=0, rr=w18, cs=w26; i<3; ++i) (*rr++) -= (*cs++);
  /* #73: @18 = (2.*@18) */
  for (i=0, rr=w18, cs=w18; i<3; ++i) *rr++ = (2.* *cs++ );
  ...
  casadi_fill(w10, 3, 10.);
  /* #77: @18 = @14[:3] */
  for (rr=w18, ss=w14+0; ss!=w14+3; ss+=1) *rr++ = *ss;
  /* #78: @26 = @22[:3] */
  for (rr=w26, ss=w22+0; ss!=w22+3; ss+=1) *rr++ = *ss;
  /* #79: @18 = (@18-@26) */
  for (i=0, rr=w18, cs=w26; i<3; ++i) (*rr++) -= (*cs++);
  /* #80: @18 = (2.*@18) */
  for (i=0, rr=w18, cs=w18; i<3; ++i) *rr++ = (2.* *cs++ );
  ...
  casadi_fill(w10, 3, 10.);
  /* #85: @18 = @15[3:6] */
  for (rr=w18, ss=w15+3; ss!=w15+6; ss+=1) *rr++ = *ss;
  /* #86: @26 = @23[3:6] */
  for (rr=w26, ss=w23+3; ss!=w23+6; ss+=1) *rr++ = *ss;
  /* #87: @18 = (@18-@26) */
  for (i=0, rr=w18, cs=w26; i<3; ++i) (*rr++) -= (*cs++);
  /* #88: @18 = (2.*@18) */
  for (i=0, rr=w18, cs=w18; i<3; ++i) *rr++ = (2.* *cs++ );
  ...
  casadi_fill(w10, 3, 10.);
  /* #92: @18 = @15[:3] */
  for (rr=w18, ss=w15+0; ss!=w15+3; ss+=1) *rr++ = *ss;
  /* #93: @26 = @23[:3] */
  for (rr=w26, ss=w23+0; ss!=w23+3; ss+=1) *rr++ = *ss;
  /* #94: @18 = (@18-@26) */
  for (i=0, rr=w18, cs=w26; i<3; ++i) (*rr++) -= (*cs++);
  /* #95: @18 = (2.*@18) */
  for (i=0, rr=w18, cs=w18; i<3; ++i) *rr++ = (2.* *cs++ );
  ...
  casadi_fill(w10, 3, 10.);
  /* #100: @18 = @16[3:6] */
  for (rr=w18, ss=w16+3; ss!=w16+6; ss+=1) *rr++ = *ss;
  /* #101: @26 = @24[3:6] */
  for (rr=w26, ss=w24+3; ss!=w24+6; ss+=1) *rr++ = *ss;
  /* #102: @18 = (@18-@26) */
  for (i=0, rr=w18, cs=w26; i<3; ++i) (*rr++) -= (*cs++);
  /* #103: @18 = (2.*@18) */
  for (i=0, rr=w18, cs=w18; i<3; ++i) *rr++ = (2.* *cs++ );
  ...
  casadi_fill(w10, 3, 10.);
  /* #107: @18 = @16[:3] */
  for (rr=w18, ss=w16+0; ss!=w16+3; ss+=1) *rr++ = *ss;
  /* #108: @26 = @24[:3] */
  for (rr=w26, ss=w24+0; ss!=w24+3; ss+=1) *rr++ = *ss;
  /* #109: @18 = (@18-@26) */
  for (i=0, rr=w18, cs=w26; i<3; ++i) (*rr++) -= (*cs++);
  /* #110: @18 = (2.*@18) */
  for (i=0, rr=w18, cs=w18; i<3; ++i) *rr++ = (2.* *cs++ );
  ...
  casadi_fill(w10, 3, 10.);
  /* #115: @18 = @17[3:6] */
  for (rr=w18, ss=w17+3; ss!=w17+6; ss+=1) *rr++ = *ss;
  /* #116: @26 = @25[3:6] */
  ...
  /* #128: @5 = vertcat(@3, @19, @20, @21, @22, @23, @24) */
  rr=w5;
  for (i=0, cs=w3; i<6; ++i) *rr++ = *cs++;
...
static int casadi_f1(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  casadi_fill(w11, 3, 10.);
  /* #47: @19 = @15[3:6] */
  for (rr=w19, ss=w15+3; ss!=w15+6; ss+=1) *rr++ = *ss;
  /* #48: @19 = (2.*@19) */
  for (i=0, rr=w19, cs=w19; i<3; ++i) *rr++ = (2.* *cs++ );
  /* #49: @11 = (@11*@19) */
  for (i=0, rr=w11, cs=w19; i<3; ++i) (*rr++) *= (*cs++);
  ...
  casadi_fill(w11, 3, 10.);
  /* #52: @19 = @15[:3] */
  for (rr=w19, ss=w15+0; ss!=w15+3; ss+=1) *rr++ = *ss;
  /* #53: @19 = (2.*@19) */
  for (i=0, rr=w19, cs=w19; i<3; ++i) *rr++ = (2.* *cs++ );
  /* #54: @11 = (@11*@19) */
  for (i=0, rr=w11, cs=w19; i<3; ++i) (*rr++) *= (*cs++);
  ...
  casadi_fill(w11, 3, 10.);
  /* #58: @19 = @16[3:6] */
  for (rr=w19, ss=w16+3; ss!=w16+6; ss+=1) *rr++ = *ss;
  /* #59: @19 = (2.*@19) */
  for (i=0, rr=w19, cs=w19; i<3; ++i) *rr++ = (2.* *cs++ );
  /* #60: @11 = (@11*@19) */
  for (i=0, rr=w11, cs=w19; i<3; ++i) (*rr++) *= (*cs++);
  ...
  casadi_fill(w11, 3, 10.);
  /* #63: @19 = @16[:3] */
  for (rr=w19, ss=w16+0; ss!=w16+3; ss+=1) *rr++ = *ss;
  /* #64: @19 = (2.*@19) */
  for (i=0, rr=w19, cs=w19; i<3; ++i) *rr++ = (2.* *cs++ );
  /* #65: @11 = (@11*@19) */
  for (i=0, rr=w11, cs=w19; i<3; ++i) (*rr++) *= (*cs++);
  ...
  casadi_fill(w11, 3, 10.);
  /* #69: @19 = @17[3:6] */
  for (rr=w19, ss=w17+3; ss!=w17+6; ss+=1) *rr++ = *ss;
  /* #70: @19 = (2.*@19) */
  for (i=0, rr=w19, cs=w19; i<3; ++i) *rr++ = (2.* *cs++ );
  /* #71: @11 = (@11*@19) */
  for (i=0, rr=w11, cs=w19; i<3; ++i) (*rr++) *= (*cs++);
  ...
  casadi_fill(w11, 3, 10.);
  /* #74: @19 = @17[:3] */
  for (rr=w19, ss=w17+0; ss!=w17+3; ss+=1) *rr++ = *ss;
  /* #75: @19 = (2.*@19) */
  for (i=0, rr=w19, cs=w19; i<3; ++i) *rr++ = (2.* *cs++ );
  /* #76: @11 = (@11*@19) */
  for (i=0, rr=w11, cs=w19; i<3; ++i) (*rr++) *= (*cs++);
  ...
  casadi_fill(w11, 3, 10.);
  /* #80: @19 = @18[3:6] */
  for (rr=w19, ss=w18+3; ss!=w18+6; ss+=1) *rr++ = *ss;
  /* #81: @19 = (2.*@19) */
  for (i=0, rr=w19, cs=w19; i<3; ++i) *rr++ = (2.* *cs++ );
  /* #82: @11 = (@11*@19) */
  for (i=0, rr=w11, cs=w19; i<3; ++i) (*rr++) *= (*cs++);
  ...
  /* #89: @8 = vertcat(@10, @12, @13, @14, @15, @16, @17) */
  rr=w8;

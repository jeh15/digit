...
def main(argv=None):
    ...
    task_space_transform, velocity_jacobian, bias_acceleration = dynamics_utilities.calculate_task_space_terms(
        plant=plant,
        context=plant_context,
    ...
    while current_time < end_time:
        ...
        # Send command:
        right_leg_control = torque[actuation_indx]

        print(torque)

        augmented_torque = np.concatenate([left_leg_control, left_arm_control, right_leg_control, right_arm_control])
        torque_command = digit_idx.actuation_map(augmented_torque)
